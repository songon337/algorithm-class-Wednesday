...
class CircularQueueOneSlotEmpty:
    ...
    def  __init__(self,capacity):
        ...
        self.N = self.capacity+1 # 내부배열의 크기 
        self.array = [None] * self.N # 배열
        self.front = 0 #프론트  
        self.rear = 0 #리어
    def is_empty(self):
        return self.front == self.rear
    def is_full(self):
        return self.front == ((self.rear+1)% self.N ) # front가 rear의 다음에 있으면 포화상태
    
    def enqueue(self, item):
        if not self.is_full():
            self.rear = (self.rear+1) % self.N  # rear 값을 먼저 하나 증가()
